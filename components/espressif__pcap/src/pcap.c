...
esp_err_t pcap_capture_packet(pcap_file_handle_t pcap, void *payload, uint32_t length, uint32_t seconds, uint32_t microseconds)
{
    ...
    ESP_RETURN_ON_FALSE(pcap->file, ESP_ERR_INVALID_ARG, TAG, "file pointer is NULL");
    ...
    printf("Seconds: %lu\n\r", header.seconds);
    printf("Microseconds: %lu\n\r", header.microseconds);
    printf("Capture Length: %lu\n\r", header.capture_length);
    printf("Packet Length: %lu\n\r", header.packet_length);
